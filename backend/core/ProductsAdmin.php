        ...
        if($this->request->method('post')) {
            ...
            if(!empty($ids)) {
                switch($this->request->post('action')) {
					...
					//	Тестовое задание, начало
					case 'set_is_radio': {
                        /*Включить "Отображение через Radio"*/
                        $this->products->update_product($ids, array('is_radio'=>1));
                        break;
                    }
					//	Тестовое задание, начало
					
					case 'unset_is_radio': {
                        /*Выключить "Отображение через Radio"*/
                        $this->products->update_product($ids, array('is_radio'=>0));
                        break;
                    }
                    case 'set_feed': {
                        /*Выгружать в фид*/
